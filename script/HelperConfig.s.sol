...
contract CONFIG is Script {
    ...
    constructor() {
        ...
        config[ZK_SEP_CHAINID] = Config({entryPoint: address(0), acc: ACCOUNT});
    }

    /* -------------------------------- functions ------------------------------- */
    Config public localConfig;

    function getConfigByChainID() public view returns (Config memory) {
    ...
    function getOrCreateAnvilConfig() public view returns (Config memory) {
        if (localConfig.entryPoint != address(0)) {
            ...
            return Config({entryPoint: address(0), acc: ANVIL_ACC});
        }
    }
}
